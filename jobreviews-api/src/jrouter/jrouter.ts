...
type Method = {
	type: "expressRouter"|"subRouter"|"get"|"post"|"put"|"delete";
	path:string;
	subRouter?:SubRouter;
	...
	_prepareExpressRouter = (router:Express|Router,errorHandler:Function) => {
		...
		for(const { type,path,validation,method,subRouter,expressRouter } of this._methods) {
			if(subRouter) {
				...
				switch(type) {
					// @ts-ignore
					case "get": router.get(path,executor);
					break;
					// @ts-ignore
					case "post": router.post(path,executor);
					break;
					// @ts-ignore
					case "put": router.put(path,executor);
					break;
					// @ts-ignore
					case "delete": router.delete(path,executor);
					break;
...
class SubRouter extends _JRouter {
	...
	useDeleteMethod = (path:string,validation:Schema,method:RestHandler) => this._methods.push({ type:"delete",path,validation,method });
