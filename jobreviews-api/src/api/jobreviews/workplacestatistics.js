...
const workplaceStatistics = (jobReviewsApi) => {
	const router = new SubRouter();
	
	/*
	router.usePostMethod("/",null,async(request,response) => {
		const { body } = await jobReviewsApi.workplace_POST(request);
		response.send(body);
	});
	*/

	router.useGetMethod("/:_id",null,async(request,response) => {
		const { body } = await jobReviewsApi.workplaceStatistics_GET(request);
		response.set("Cache-Control","public, max-age=31557600"); // one year
		response.send(body);
	});
